...
This was re-verified after the extension became active in that same session: `org.kde.StatusNotifierWatcher` appeared on the session bus, PortPatch's own item showed up in its `RegisteredStatusNotifierItems` (as `org/chromium/StatusNotifierItem/...`, Electron's underlying implementation), and the icon was visually confirmed rendering in the GNOME top bar. So the tray implementation itself is not the problem; whether the icon appears depends entirely on that extension's state, which PortPatch has no way to detect or influence.
...
The executable path written into that entry comes from `resolveLinuxExecutablePath()`, which prefers `process.env.APPIMAGE` over `process.execPath`. This matters specifically because a running AppImage's `process.execPath` resolves to a path under a per-launch `/tmp/.mount_*` FUSE mount (confirmed directly against a running instance: `/tmp/.mount_PortPa.../portpatch`), which stops existing once that instance exits; an autostart entry written with it would point at nothing after the next reboot. The AppImage runtime sets `APPIMAGE` to the original file's stable path before exec'ing the wrapped binary (also confirmed directly: it resolved to the real `release/PortPatch-*.AppImage` path), which is what actually needs to be launched again at the next sign-in.
